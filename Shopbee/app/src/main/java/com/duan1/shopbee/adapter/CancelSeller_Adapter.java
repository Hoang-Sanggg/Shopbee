...
public class CancelSeller_Adapter extends RecyclerView.Adapter<CancelSeller_Adapter.OrderHoder> {
...
//    private ClickToProductSale clickToProductSale;
    ...
    public CancelSeller_Adapter(List<Order> orderList, Context context, ClickToOrder clickToOrder, ClickNextStatus clickNextStatus) {
    ...
    @NonNull
    @Override
    public CancelSeller_Adapter.OrderHoder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_pending_product, parent, false);
        return new CancelSeller_Adapter.OrderHoder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull CancelSeller_Adapter.OrderHoder holder, int position) {

        SharedPreferences sharedPref = context.getSharedPreferences("MyPreferences", Context.MODE_PRIVATE);
        String nameShopS = sharedPref.getString("username", "");

        if(String.valueOf(orderList.get(position).getSeller()).equals(nameShopS)==true && String.valueOf(orderList.get(position).getCancelSeller()).equals("1")==true){
...
//            }
//        });
        holder.btnDeliveryPending.setText("Xác nhận");
