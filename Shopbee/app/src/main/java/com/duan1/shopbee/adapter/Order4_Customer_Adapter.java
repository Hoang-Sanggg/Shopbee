...
public class Order4_Customer_Adapter extends RecyclerView.Adapter<Order4_Customer_Adapter.OrderHoder> {
...
//    private ClickToProductSale clickToProductSale;
    ...
    public Order4_Customer_Adapter(List<Order> orderList, Context context, ClickToOrder clickToOrder, ClickNextStatus clickNextStatus) {
    ...
    @NonNull
    @Override
    public Order4_Customer_Adapter.OrderHoder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_pending_product, parent, false);
        return new Order4_Customer_Adapter.OrderHoder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull Order4_Customer_Adapter.OrderHoder holder, int position) {

        SharedPreferences sharedPref = context.getSharedPreferences("MyPreferences", Context.MODE_PRIVATE);
        String nameShopS = sharedPref.getString("username", "");

        if(String.valueOf(orderList.get(position).getCustomer()).equals(nameShopS)==true && String.valueOf(orderList.get(position).getStatusOrder()).equals("4")==true){
...
//            }
//        });
        holder.btnDeliveryPending.setText("Đã giao");
